...
int HandlerCGIFather(int new_sock,int father_read,int father_write,int child_pid,Request*req){
	//1.如果请求是POST，就把body写到管道中
	//由于对new_sock的读取在程序的前面已经度过很多次了
	// 并且当前缓冲区中如果有数据，那么一定是body部分的数据，直接读取
	...
	{
		//管道写端全部关闭，再尝试继续读，就会返回0；
		//管道读端全部关闭，再尝试继续写，就会产生SIGPIPE信号
		send(new_sock,&c,1,0);
	}
	//4.回收子进程的资源
	//wait(NULL);//多线程不支持（）无差别等待子进程返回
	waitpid(child_pid,NULL,0);//fork()函数返回的就是子进程的pid.
	return 200;
}
...
int HandlerCGIChild(int child_read,int child_write,Request* req)
{
	...
	sprintf(method_env,"REQUERY_METHOD=%s",req->method);
	putenv(method_env);
	//还需要设置QUERY_STRING,CONTEN-LENGTH
	if(strcasecmp(req->method,"GET")==0)
	{
		...
		char content_length_str[SIZE]={0};
		sprintf(content_length_str,"CONTENT_LENGTH=%d",req->content_length);
		putenv(content_length_str);
	}
	//b)把标准输入输出重定向到管道中
	dup2(child_read,0);
	dup2(child_write,1);
	...
	//c)程序替换
	//url-path:/cgi-bin/test
	//file-path:./wwwroot/cgi-bin/test
...
int HandlerCGI(int new_sock,Request*req)
{
	...
	//提高变量的可读性，给变量起一个有实际意义的名字
	int father_read=fd1[0];//father从fd1管道读   child从fd1管道写
	int child_write=fd1[1];
	int child_read=fd2[0];//father从fd2管道写   child从fd2管道读
	...
	if(ret>0){
		//father
		//此处父进程优先关闭着2个管道的文件描述符，是为了后续父进程从子进程中读取
		//数据时能够读取到 EOF，对于管道来说，所有的写端关闭，继续读，才有 EOF 。
		//而此处的所有写端，一方面父进程需要关闭，
		//另一方面子进程也需要关闭.所以此处父进程先关闭不必要的写端后，
		//后续子进程用完了直接关闭，父进程也就读到了EOF
		close(child_read);
		close(child_write);
